...
\begin{abstract}
  ...
  access sites on the .onion domain.  The focus of .onion use
  ...
  certificates---provided that changes are made to the current rules
  that limit the kinds of certificates permissible for .onion
  addresses.  We explain why, in the context of our design, those
...
and other guarantees more simply, easily, fully, and cheaply
...
wishing to connect to one's site via Tor (e.g., skirting exit relay
bandwidth limitations). And Tor proposals (the Tor equivalent of IETF
RFCs) to standardize further simplified onion services without
...
web locations is also possible.
...
%As of this writing, Let's Encrypt services do not provide certificates
%for onionsites.
%\griffin I decided to leave this section in but change it slightly
% I cut that sentence, Implied by things elsewhere, and madly hunting
% to get below 6000 words. -PFS
...
%connect the onionsite to some pseudonymous reputation.  
Authenticated
...
%while we would prefer to leave this disclosure at the discretion of
%the onion service operator.
...
%We reiterate that meaningful
%names are important, and a registered domain is an understood way to
%achieve this.  Even if someone registers a human-meaningful
%domain name to bind the onion address to, however, s/he need not actually run a
%server at that address; the onionsite is enough for these
%purposes. The GPG-signed-onion technique allows someone to have
%a desired domain name, while still being able to
%offer an authenticated and integrity protected site easily. 
%
%This also illustrates one of the benefits of using GPG or similar
%signatures, regardless of whether the owner of the registered domain
%sets up a service at that address or not. If the authentication simply
%showed that the same party that controlled the registered domain
%provided the onionsite, an attacker could set up his own site, employ
%usual techniques to hijack the insecure site if it exists, and offer a
%self-authenticated onionsite that matches his alternative to the
%hijacked site.  To do this undetectably against the GPG-signed
%onionsite would require subversion of the trust in the GPG key. 
%A concern with using GPG signatures is that users may not be familiar
...
% Though installation is a simple
% point-click-run download, some may perceive this process as being
% difficult or may have network restrictions that make using Tor more
% difficult. 
An onionsite could typically still be accessed via Tor2web, a website
that proxies connections from non-Tor clients to
onionsites~\cite{tor2web}.  Whatever availability such proxying
...
But, it is not just the little guy. We have already noted some of the
...
(Though a much more significant difference from current HTTPS
Everywhere, note that the ruleset could be expanded allow redirection
...
for general adoption of the stronger and broader authentication and
confidentiality offered by onionsites in the years to come. We also
hope our expanded view of the possibilities created by Tor's onion
services will encourage others to explore this fascinating system for
